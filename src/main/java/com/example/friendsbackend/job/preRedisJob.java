...
//    });
    ...
    public void doCacheRecommendUser(){
        RLock lock = redissonClient.getLock("gamefriend-d:preRedis:docache:lock");
        try {
            if (lock.tryLock(0,-1, TimeUnit.MICROSECONDS)){
                ...
                for (Long userid: userList){
                    ...
                    String rediasKey = String.format("gamefriend-d:user:recommend:%s",userid);
    ...
    public void deleteOverdueUser(){
        RLock lock = redissonClient.getLock("gamefriend-d:user:deleteOverdue:lock");
        try {
            if (lock.tryLock(0,-1,TimeUnit.MICROSECONDS)){
                // 删除缓存中一个月以前的用户
...
//                queryWrapper.orderByDesc("lastTime");
//                queryWrapper.last("limit 0,1000");
//                String redisKey = "gamefriend-d:user:recentHour:{id,lastTime}:String";
